...
export default function MarketNews() {
  useSEO({
    ...
    description: 'Live financial market news and insights for stocks, crypto, forex, and commodities. Real-time trading news with sentiment analysis on Trade AI Zotra.',
    keywords: 'market news, financial news, stock market news, crypto news, trading insights, market sentiment, Trade AI Zotra news',
  });
  ...
  const { data: newsData, isLoading, isFetching } = useQuery({
    ...
    queryFn: async () => {
      const result = await base44.integrations.Core.InvokeLLM({
        prompt: `You are a financial news aggregator with live web access. Use web search to find 12 REAL current trading and financial market news articles from today. Use ACTUAL headlines, REAL sources (Reuters, Bloomberg, CNBC, CoinDesk, WSJ, etc.), and the REAL article URL when available. Do not invent headlines, sources, or URLs. Include a mix of stocks, crypto, forex, commodities, and macro economy news.
    ...
    <div className="p-6 md:p-8 max-w-6xl mx-auto">
      ...
      >
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">AI-synthesized market briefing · grounded in live web search</p>
          <p className="text-[10px] font-mono text-amber-400/70 mt-1">⚠️ Headlines &amp; summaries are AI-generated from live web sources. Verify with the original source before acting.</p>
        </div>
        <Button
